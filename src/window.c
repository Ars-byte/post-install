...
static void go_to_page(GtkButton *btn, gpointer user_data) {
...
static GtkWidget* create_app_group_page(NekoStoreWindow *self, const char *title, AppGroup group_filter, const char *back_id, const char *next_id, const char *next_btn_label, GCallback next_cb) {
...
static gboolean on_language_switch_state_set(GtkSwitch *widget, gboolean state, gpointer user_data) {
    ...
    GtkWidget *btn = gtk_widget_get_last_child(main_content);
...
static void neko_store_window_init (NekoStoreWindow *self) {
    ...
    for (int i = 0; i < G_N_ELEMENTS(defs); i++) {
        ...
        const char *next_id = (i == G_N_ELEMENTS(defs) - 1) ? NULL : defs[i+1].id;
        const char *btn_label = (i == G_N_ELEMENTS(defs) - 1) ? "Install" : "Next";
        GCallback next_cb = (i == G_N_ELEMENTS(defs) - 1) ? G_CALLBACK(on_install_selected_clicked) : G_CALLBACK(go_to_page);
        
        self->step_pages[i] = create_app_group_page(self, defs[i].title, defs[i].group, back_id, next_id, btn_label, next_cb);
        gtk_stack_add_named(GTK_STACK(self->stack), self->step_pages[i], defs[i].id);
    }
