...
static void on_line_read(GObject *source_object, GAsyncResult *res,
                         gpointer user_data) {
  (void)source_object;
  ...
  GError *error = NULL;
  char *line =
      g_data_input_stream_read_line_finish(task->stream, res, &length, &error);
  ...
  if (line != NULL) {
    ...
    g_free(line);
    g_data_input_stream_read_line_async(task->stream, G_PRIORITY_DEFAULT, NULL,
                                        on_line_read, task);
  } else {
    if (error) {
      g_clear_error(&error);
...
static void on_process_finished(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  InstallTask *task = user_data;
  GError *error = NULL;
  gboolean success =
...
void install_app_async(const char *command, InstallProgressCallback progress_cb,
                       InstallFinishedCallback finished_cb,
                       gpointer user_data) {
  ...
  GError *error = NULL;
  task->subprocess = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_PIPE |
                                          G_SUBPROCESS_FLAGS_STDERR_MERGE,
                                      &error, "bash", "-c", command, NULL);
